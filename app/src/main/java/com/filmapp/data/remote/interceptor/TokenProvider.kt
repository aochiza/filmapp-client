...
class TokenProvider @Inject constructor(
    ...
    fun getUserId(): Flow<Int?> = context.dataStore.data
        .map { it[USER_ID_KEY]?.toIntOrNull() }

    fun getName(): Flow<String?> = context.dataStore.data
        .map { it[NAME_KEY] }

    suspend fun saveAuthData(token: String, userId: Int, name: String) {
    ...
    fun isLoggedIn(): Flow<Boolean> = getToken().map { !it.isNullOrEmpty() }


    fun getTokenSync(): String? = runBlocking { getToken().first() }
}